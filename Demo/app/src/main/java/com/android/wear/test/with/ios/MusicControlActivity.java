...
// .build();
        ScanFilter filter = new ScanFilter.Builder().setServiceUuid(ParcelUuid.fromString(service_ams))
                .build();
...
//            Log.d(TAG_LOG, skip_count + "judge: " + result.getDevice().getAddress().toString().equals
// (iphone_uuid));
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (!is_set_entity) {
                    ...
                    BluetoothGattCharacteristic characteristic = service.getCharacteristic(UUID.fromString
                            (characteristics_remote_command));
...
//                    if ("0001".equals(characteristic.getUuid().toString())) {
                    ...
                    try {
                        ...
                        BluetoothGattService service = bluetooth_gatt.getService(UUID.fromString
                                (service_ams));
                        if (service != null) {
                            BluetoothGattCharacteristic chara = service.getCharacteristic(UUID.fromString
                                    (characteristics_entity_update));
                            if (chara != null) {
                                bluetooth_gatt.setCharacteristicNotification(chara, true);
                                BluetoothGattDescriptor desc = chara.getDescriptor(UUID.fromString
                                        (descriptor_config));
                                if (desc != null) {
